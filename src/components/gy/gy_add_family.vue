<template>
	<div class="app">
		...
		<div class="body">
			...
			<mt-cell title='*证件号码'>
				<input class='input-box' @input='validate($event)' @change='validateForm($event)' type="text" placeholder="请输入证件号码" ref="gy_hdzhi"/>
			</mt-cell>
			<div class="gy_butn">
		...
		methods: {
			...
			gy_djcxyz() {
				if(this.cardType == "居民身份证") {
					if(/^[1-9]\d{7}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}$|^[1-9]\d{5}[1-9]\d{3}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}([0-9]|X)$/.test(this.$refs.gy_hdzhi.value)) {
						// console.log("身份证正确")
						var birthdayStr = this.$refs.gy_hdzhi.value.substring(6, 14);
				...
				if(this.cardType == "台湾居民来往内地通行证") {
					if(/[A-Z][0-9]{9}/.test(this.$refs.gy_hdzhi.value)) {
				...
				if(this.cardType == "港澳居民来往内地通行证") {
					if(/^[HMhm]{1}([0-9]{10}|[0-9]{8})$/.test(this.$refs.gy_hdzhi.value)) {
						sessionStorage.removeItem("otherBirthday");
					} else {
						MessageBox('提示', '身份证信息有误');
						return;
				...
				if(this.cardType == "护照") {
					if(/^[a-zA-Z]{5,17}$/.test(this.$refs.gy_hdzhi.value) | /^[a-zA-Z0-9]{5,17}$/.test(this.$refs.gy_hdzhi.value)) {
						sessionStorage.removeItem("otherBirthday");
					} else {
						MessageBox('提示', '身份证信息有误');
						return;
				...
				sessionStorage.setItem("certificateNo", this.$refs.gy_hdzhi.value);
