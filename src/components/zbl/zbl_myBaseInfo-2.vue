...
<div class="app" style="overflow-x:hidden;height:100vh;">
	...
	<div class="body">
		...
		<mt-field label="*证件号码" v-model='zbl_certificateNo'></mt-field>
		...
		<mt-field label="*手机号码" v-model='zbl_phoneNo'></mt-field>
...
export default {
					...
					if (document.querySelector('.sexType').innerText != '请选择') {
						if (document.querySelector('.birthDay').innerText != '请选择') {
							if (this.zbl_phoneNo != '请输入') {
								...
								if (/^[1-9]\d{7}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}$|^[1-9]\d{5}[1-9]\d{3}((0\d)|(1[0-2]))(([0|1|2]\d)|3[0-1])\d{3}([0-9]|X)$/.test(this.zbl_certificateNo)) {
									if (this.zbl_phoneNo) {
										if (/^1[34578]\d{9}$/.test(this.zbl_phoneNo)) {
											commonAjaxKy(JSON.stringify(params), 'hcn.person', 'saveSelfInfo').then(res => {
												if (res.code === 200) {
													console.log(res)
													// JSON.parse(sessionStorage.getItem("userInfo")).body.userName = this.zbl_personName
													let obj = {}
													obj.code = JSON.parse(sessionStorage.getItem("userInfo")).code
													let obj2 = JSON.parse(sessionStorage.getItem("userInfo")).body
													obj2.userName = this.zbl_personName
													obj.body = obj2
													obj.properties = JSON.parse(sessionStorage.getItem("userInfo")).properties
													console.log(obj);
	...
	beforeRouteEnter(to,from,next) {
		// console.log(JSON.parse(sessionStorage.getItem("userInfo")).body.userName);
		if(JSON.parse(sessionStorage.getItem("userInfo")).body.userName) {
			next({path: '/mybaseinfo'})
		} else {
			next()
		}
	}
