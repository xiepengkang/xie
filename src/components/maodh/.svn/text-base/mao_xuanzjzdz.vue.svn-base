<template>
    <div class="mao_xzjzdzaddress">
        <mt-header fixed title="居住地址">
...
<script>
import { Navbar,TabItem,Header,Button,Cell,Field,DatetimePicker,MessageBox,Picker,Popup,Toast } from 'mint-ui';
import "../../assets/css/mao_style.css";
import provinceCityArea from "../json/provinceCityArea.json";
import {zbl_cityPickerIndex} from '../../assets/js/zbl_cityPickerIndex.js';
import {areaAjax} from '../../api/api'
    export default {
        data: function() {
            return {
                howntown: '',
        ...
        methods: {
            saveAddress() {
                if (this.$refs.selectCity.innerHTML && this.$refs.selectStreet.innerHTML && this.howntown) {
                    ...
                    this.maobaocunjiedao=obj.decText
                    // console.log(this.maobaocunjiedao.decText)
                    sessionStorage.setItem("maoxzjmdz",this.maobaocunjiedao);
                    this.$store.dispatch('mao_select_shiqubm',obj)
                    console.log(this.maobaocunjiedao)
...
.szArea {
    ...
    .mao_xzjzdzaddress{width:100%;height:100%;}
    .mao_xzjzdzaddress .mint-header{background-color:#fff;color:#26a2ff;font-size:0.33rem;z-index:5;height:1rem;border-bottom:1px solid #eee;border-bottom:.2rem solid #ddd;}
    .mao_xzjzdzaddress .mao_mxzdq{clear:both;padding-top:1.2rem;height:100%;}
</style>