...
//			targetUserId:this.$route.query.targetUser,
	...
	activated() {
		...
		if (this.targetType == "041") {//当前人是医生
			...
			this.operateUserName = userInfo.name
			this.operateId = userInfo.id
			this.accessToken = userInfo.roles[0].accessToken
		} else {//当前人是患者
	...
	methods: {
		...
		getOperater(callback) {
			commonAjax([this.operateId, ""], 'pcn.myResidentDoctorService', 'personalInfo').then(res => {
				var operateUser = res.body.docInfo.doctorId
		...
		saveMsg(params) {
			...
			me.getOperater(function (operateUser) {
				commonAjax([{
					...
					operaterType: me.operaterType,
//					targetUser:me.targetUserId,
					targetUser: me.targetUser,
