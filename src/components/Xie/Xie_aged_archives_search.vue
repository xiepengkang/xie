	...
	<div class="lnrda">
		...
		<div class="search_wrap1" >
				...
				<div class="search_height">
					<mt-search  placeholder="请输入姓名、身份证、网格地址搜索" cancel-text="" v-model='searchs' ></mt-search>
				</div>
		</div>
		<!--预约列表-->
		<ul class="main1">
...
//  }),
    methods: {
			xie_getlist() {
                ...
                let params = [JSON.parse(sessionStorage.getItem('xie_localDoctorId'))[0],JSON.parse(sessionStorage.getItem('xie_localDoctorId'))[1],this.searchs,1,50]
                commonAjax(params, 'pcn.archiveService', 'queryOldPeopleArchiveList').then(res => {
                     if (res.code == 200) {
                     console.log(res)	
					this.list=res.body;
//					筛选50岁以上的档案
//					res.body.forEach(list_aged=>{
//						if(list_aged.age>=50){
//							this.list.push(list_aged)
