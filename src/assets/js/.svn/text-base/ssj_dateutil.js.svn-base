...
function getDateStr(date,addDayCount) {
	...
	dateNew.setDate(dateNew.getDate() + addDayCount);//获取AddDayCount天后的日期
	return dateNew;
 ...
 * 距离现在的时间差
 */
function getDateDiff2Now(startTime) {
	var now = new Date();  // 开始时间
	if(getDateDiff(startTime, now, "m")<60){
		if(getDateDiff(startTime, now, "m")>=1)
			return getDateDiff(startTime, now, "m") + " 分钟前";
		return "1 分钟前";
	}
	if(getDateDiff(startTime, now, "h")<24)
		return getDateDiff(startTime, now, "h") + " 小时前";
	return getDateDiff(startTime, now, "d") + " 天前";
}
