...
export default {
	...
	methods: {
		getAllNotificationCount() {
			//          	Indicator.open('加载中...');
			//let businessTypes = ["002","616","014","088","520","613"];
			//  			let msgTitleArr = ["家庭管理","家医通知","报告历史查询", "家医签约","咨询互动", "服务记录"];
			//  			let imgPathArr = ["zy_family_manage.png", "zy_family_notification.png","zy_report_history.png","zy_sign.png","zy_consult.png","zy_service_record.png"];
			//  			let routePathArr = ["zy_family_manage", "zy_family_notification","zy_report_history","zy_doc_sign","zy_consult","zy_service_record"];
			...
			commonAjaxKy(JSON.stringify(params), 'hcn.notification', 'getAllNotificationCount').then(res => {
				...
				if (res.code == 200) {
					...
					//
					//							let listobj = this.list1[i]
					//							console.log(listobj)
					//							if(item.businessType==listobj.businessType){
					//
					//								item.xie_p=listobj.xie_p
					//								item.xie_img=listobj.xie_img
					//								item.xie_go=listobj.xie_go
					//								item.xie_format=this.formatMsgTime (this.milliseconds)
					//							}
					//						}
					//					})


					//					console.log(require('listobj.xie_img'))
					//					this.list=res.body
					//					this.list.forEach(item=>{
					//						this.milliseconds=Date.parse(new Date())-Date.parse(item.lastNotification.sendTime)
					//
					...
					//					this.list.forEach(item =>{
					//					this.milliseconds=Date.parse(new Date())-Date.parse(item.lastNotification.sendTime)
					//					 this.formatMsgTime (this.milliseconds)
					//					 this.listtime.push(this.formatMsgTime(this.milliseconds))
					////
					//					})
